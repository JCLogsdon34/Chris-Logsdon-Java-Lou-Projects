 ...
 * @author JCLog
 */
public class DaoOrderProductionImpl implements DaoOrder {
    ...
    public Order removeOrder(String orderDate, String orderNumber) {
        ...
        currentOrder = dayOrders.remove(i);
            orders.remove(orderDate, dayOrders);
      //   if (dayOrders.size() > 0){
...
//
    ...
    public void writeOrder() throws FlooringPersistenceException, FlooringBadDateException {
        ...
        for (String date : orders.keySet()) {
            ...
            o = theFile.replace("OrderFile\\Orders_", "");
            orderDate = o.replace(".txt", "");
            fileName = getMyFile(date);
            List<Order> orderList = new ArrayList<>();
            try {
                out = new PrintWriter(new FileWriter(fileName));
            } catch (IOException e) {
                throw new FlooringPersistenceException("Could not write files", e);
            }

            orderList.addAll(getAllOrdersByDate(date));

            for (Order currentOrder : orderList) {
                out.println(currentOrder.getOrderNumber() + DELIMITER
                        + currentOrder.getOrderName() + DELIMITER
